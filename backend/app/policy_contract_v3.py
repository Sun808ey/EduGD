...
def validate_policy_v3(value: object) -> dict[str, object]:
    ...
    if not isinstance(value, dict) or set(value) != {
        "schema_version", "timezone", "refresh_after_seconds", "default_mode",
        "emergency_packages", "required_capabilities", "minimum_dpc_version",
        "modes", "schedules", "screen_time", "web_filter",
    }:
    ...
    common = validate_policy_v2({key: value[key] for key in (
        "schema_version", "timezone", "refresh_after_seconds", "default_mode",
        "emergency_packages", "required_capabilities", "minimum_dpc_version", "modes", "schedules")}
        | {"schema_version": 2})
    screen = value["screen_time"]
    if not isinstance(screen, dict) or set(screen) != {"daily_limit_minutes", "reset_minute", "exhausted_mode_id"}:
    ...
    if not isinstance(exhausted, str) or exhausted not in {m["mode_id"] for m in common["modes"]}:
        raise PolicyContractError("invalid exhausted_mode_id")
    web = value["web_filter"]
    if not isinstance(web, dict) or set(web) != {"default_action", "rules"} or web["default_action"] != "allow":
    ...
    for raw in web["rules"]:
        if not isinstance(raw, dict) or set(raw) != {"rule_id", "domain", "action", "reason"}:
            raise PolicyContractError("invalid web-filter rule")
        rule_id, domain, action, reason = raw.values()
        if not isinstance(rule_id, str) or not _RULE_ID.fullmatch(rule_id) or rule_id in ids:
            raise PolicyContractError("invalid web-filter rule_id")
        domain = normalize_domain(domain)
        if action not in {"allow", "block"} or not isinstance(reason, str) or not 1 <= len(reason) <= 256 or not reason.isprintable():
            raise PolicyContractError("invalid web-filter rule")
        if (domain, action) in seen:
            raise PolicyContractError("duplicate web-filter rule")
        seen.add((domain, action)); ids.add(rule_id)
        rules.append({"rule_id": rule_id, "domain": domain, "action": action, "reason": reason})
    return {**common, "schema_version": POLICY_SCHEMA_VERSION, "screen_time": {"daily_limit_minutes": limit, "reset_minute": reset, "exhausted_mode_id": exhausted}, "web_filter": {"default_action": "allow", "rules": sorted(rules, key=lambda r: r["rule_id"])}}
...
def build_policy_v3_envelope(*, policy_uuid: str, revision_uuid: str, revision_number: int, issued_at: str, signing_key_id: str, payload: object) -> dict[str, object]:
...
def sign_policy_v3_envelope(envelope: dict[str, object], private_key: Ed25519PrivateKey) -> dict[str, object]:
    signature = private_key.sign(canonical_json_bytes(envelope))
    return {**deepcopy(envelope), "signature": base64.urlsafe_b64encode(signature).rstrip(b"=").decode("ascii")}


def verify_policy_v3_envelope(envelope: object, public_key: Ed25519PublicKey) -> dict[str, object]:
    if not isinstance(envelope, dict) or set(envelope) != {"protocol_version", "policy_uuid", "revision_uuid", "revision_number", "issued_at", "payload_hash", "signing_key_id", "payload", "signature"} or envelope.get("protocol_version") != DPC_PROTOCOL_VERSION:
        raise PolicyContractError("invalid v3 policy envelope")
    try:
        signature = base64.urlsafe_b64decode(str(envelope["signature"]) + "=" * (-len(str(envelope["signature"])) % 4))
    ...
    rebuilt = build_policy_v3_envelope(policy_uuid=str(envelope["policy_uuid"]), revision_uuid=str(envelope["revision_uuid"]), revision_number=envelope["revision_number"], issued_at=str(envelope["issued_at"]), signing_key_id=str(envelope["signing_key_id"]), payload=envelope["payload"])
    if rebuilt != unsigned: raise PolicyContractError("invalid v3 policy envelope")
    return rebuilt

...
from app.models import Administrator, Policy, PolicyRevision
...
def test_concurrent_revision_creation_allocates_unique_monotonic_versions(
    ...
    with postgres_app.app_context():
        ...
        actor = Administrator(
            username=f"policy.concurrent.{uuid4().hex[:8]}",
            display_name="Policy Concurrency Administrator",
        ...
        db.session.add_all(
            [
                Policy(
                    ...
                    name="Concurrent revision policy",
                ),
                actor,
            ]
        )
    ...
    try:
        ...
        with postgres_app.app_context():
            try:
                ...
                db.session.execute(
                    delete(Administrator).where(Administrator.id == actor_id)
                )
                db.session.commit()
            except Exception:
                db.session.rollback()
